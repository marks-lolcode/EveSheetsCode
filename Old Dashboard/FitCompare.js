...
function loadSdeCache() {
  ...
  const data = sdeSheet.getRange('A2:C10000').getValues();
  
  for (let i = 0; i < data.length; i++) {
    if (data[i][0]) {
      sdeCache[data[i][0]] = data[i][2]; // typeID -> typeName
    }
  }
  ...
  try {
    if (locationId >= 60000000 && locationId <= 69999999) {
      ...
      if (structData && structData.length > 1) {
        systemId = structData[1][3];
        systemName = structData[1][1];
    ...
    for (const charName of selectedChars) {
      ...
      for (let i = 1; i < assets.length; i++) {
        ...
        if (!locationCache[locationId]) {
          Logger.log(`Looking up location ${locationId} (flag: ${locationFlag})...`);
          locationCache[locationId] = getSystemForLocation(locationId);
          uniqueLocations++;
        }
        
        const systemInfo = locationCache[locationId];
